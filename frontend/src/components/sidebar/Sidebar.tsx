            ...
            return (
              <div className="w-full mb-1" key={'menu-' + index}>
                ...
                >
                  ...
                  >
                    <div className="text-primary-900 transition-all duration-300">{item.icon}</div>
                    <Typography
                      className={clsx(
                        'ml-7 text-gray-900 transition-all duration-300 whitespace-nowrap',
                        isHovered || window.innerWidth < 1024 ? 'opacity-100' : 'opacity-0 w-0'
                      )}
                      ml={2}
                    >
                      {item.name}
              ...
              <Link to={item.path} key={index}>
                <div
                  className={clsx(
                    ...
                    isCurrentPage(item.path) ? 'bg-red-100 border-l-4 border-red-400 ' : ''
                  )}
                >
                  <div
                    className={
                      isCurrentPage(item.path) ? 'text-red-400 font-semibold' : 'text-gray-900 '
                    }
                  >
                    {item.icon}
                  </div>
