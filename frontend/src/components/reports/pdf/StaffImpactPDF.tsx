...
export const StaffImpactPDF = ({ data }: StaffImpactPDFProps) => {
  ...
  const stats = [
    {
      label: 'Correlación Ratio-Victorias',
      value: data.ratioWinRateCorrelation ? data.ratioWinRateCorrelation.toFixed(3) : 'N/A',
    },
    {
      label: 'Correlación Ratio-Puntos',
      value: data.ratioPointsCorrelation ? data.ratioPointsCorrelation.toFixed(3) : 'N/A',
    },
    {
      label: 'Ratio Staff Promedio',
      value: data.averageStaffRatio ? data.averageStaffRatio.toFixed(2) : 'N/A',
    },
  ];
  ...
  const tableHeaders = ['Equipo', 'Ratio S/J', '% Victorias', 'Pts/PJ', 'DG', 'PJ'];
    ...
    >
      ...
      <PDFSection title="Ratio Staff vs % Victorias (Top 10)">
        <PDFMultiBarChart
          data={chartData}
          bars={[
            { key: 'ratio', name: 'Ratio Staff/Jugador', color: REPORT_COLORS.primary.main },
            { key: 'winRate', name: '% Victorias', color: REPORT_COLORS.chart.green },
          ]}
          height={180}
        />
      </PDFSection>
