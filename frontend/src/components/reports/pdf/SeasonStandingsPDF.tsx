...
export const SeasonStandingsPDF = ({ data, seasonName }: SeasonStandingsPDFProps) => {
  ...
  const avgPoints = data.length > 0
    ...
    : "0";
  const topScorer = data.length > 0
    ? data.reduce((max, team) => ((team.goalsFor || 0) > (max.goalsFor || 0) ? team : max), data[0])
    : null;

  const stats = [
    { label: "Equipos en Competencia", value: totalTeams },
    { label: "Promedio de Puntos", value: avgPoints },
    { label: "Máximo Goleador", value: topScorer ? `${topScorer.teamName}: ${topScorer.goalsFor} goles` : "-" },
  ];
